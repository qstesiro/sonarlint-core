...
public class LanguageDetection {
    ...
    public LanguageDetection(Configuration config) {
        for (Language language : Language.values()) {
            var extensions = config.get(language.getFileSuffixesPropKey()).isPresent()
                ? config.getStringArray(language.getFileSuffixesPropKey())
    ...
    public Language language(InputFile inputFile) {
        ...
        if (detectedLanguage != null) {
            LOG.debug(
                "Language of file '{}' is detected to be '{}'",
                inputFile.uri(),
                detectedLanguage
            );
    ...
    private String getDetails(Language detectedLanguage) {
        return detectedLanguage + ": " +
            Arrays
            .stream(extensionsByLanguage.get(detectedLanguage))
            .collect(joining(","));
    }
