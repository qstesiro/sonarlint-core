...
public final class ConnectedSonarLintEngineImpl
    ...
    private AnalysisContext loadAnalysisContext() {
        ...
        var pluginDetails = loadingResult
            ...
            .map(
                p -> new PluginDetails(
                    p.getPlugin().getKey(),
                    p.getPlugin().getName(),
                    Optional.ofNullable(p.getPlugin().getVersion()).map(Version::toString).orElse(null),
        ...
        // 所有插件的可用规则
        // 注册所有SINGLE_ANALYSIS扩展点,除了Sensor
        var allRulesDefinitionsByKey = loadPluginMetadata(
    ...
    private static class ActiveRulesContext {
        ...
        public void includeRule(
            SonarLintRuleDefinition ruleOrTemplateDefinition,
            ServerActiveRule activeRule
        ) {
            var activeRuleForAnalysis = new ActiveRule(
                activeRule.getRuleKey(),
                ruleOrTemplateDefinition.getLanguage().getLanguageKey(),
                // 新增两个参数
                ruleOrTemplateDefinition.getInternalKey(),
                ruleOrTemplateDefinition.getDefaultSeverity()
            );
            ...
            activeRulesMetadata.put(
                activeRule.getRuleKey(),
                new ActiveRuleMetadata(
                    activeRule.getSeverity(),
                    ruleOrTemplateDefinition.getType()
                )
            );
        }

        private static Map<String, String> getEffectiveParams(
            SonarLintRuleDefinition ruleOrTemplateDefinition,
            ServerActiveRule activeRule
        ) {
            Map<String, String> effectiveParams = new HashMap<>(ruleOrTemplateDefinition.getDefaultParams());
            activeRule.getParams().forEach(
                (paramName, paramValue) -> {
                    if (!ruleOrTemplateDefinition.getParams().containsKey(paramName)) {
                        LOG.debug(
                            "Rule parameter '{}' for rule '{}' does not exist in embedded analyzer, ignoring.",
                            paramName,
        ...
        public void includeRule(SonarLintRuleDefinition rule) {
            var activeRuleForAnalysis = new ActiveRule(
                rule.getKey(),
                rule.getLanguage().getLanguageKey(),
                // 新增加两个参数
                rule.getInternalKey(),
            ...
            activeRules.add(activeRuleForAnalysis);
            activeRulesMetadata.put(
                activeRuleForAnalysis.getRuleKey(),
                new ActiveRuleMetadata(rule.getDefaultSeverity(), rule.getType())
            );
        }
    ...
    public AnalysisResults analyze(
        ...
        analysisConfigBuilder.putAllExtraProperties(
            serverConnection
                .getAnalyzerConfiguration(projectKey)
                .getSettings()
                .getAll()
        );
        ...
        var analyzeCommand = new AnalyzeCommand(
            ...
            issue -> streamIssue(issueListener, issue, activeRulesContext),
            logOutput
        );
    ...
    private void streamIssue(IssueListener issueListener, Issue newIssue, ActiveRulesContext activeRulesContext) {
        ...
        var vulnerabilityProbability = analysisContext.get()
            .findRule(newIssue.getRuleKey())
            .flatMap(SonarLintRuleDefinition::getVulnerabilityProbability);
    ...
    private ActiveRulesContext buildActiveRulesContext(ConnectedAnalysisConfiguration configuration) {
        ...
        var projectKey = configuration.getProjectKey();
        var ruleSetByLanguageKey = serverConnection
            .getAnalyzerConfiguration(projectKey)
        ...
        ruleSetByLanguageKey.entrySet()
            .stream()
            .filter(
                e -> Language.forKey(e.getKey()).filter(
                    l -> globalConfig.getEnabledLanguages().contains(l)
                ).isPresent()
            )
            .forEach(
                e -> {
                    ...
                    for (ServerActiveRule possiblyDeprecatedActiveRuleFromStorage : ruleSet.getRules()) {
                        var activeRuleFromStorage = tryConvertDeprecatedKeys(
                            possiblyDeprecatedActiveRuleFromStorage
                        );
                        SonarLintRuleDefinition ruleOrTemplateDefinition;
                        if (StringUtils.isNotBlank(activeRuleFromStorage.getTemplateKey())) {
                            ruleOrTemplateDefinition = analysisContext.get()
                                .findRule(activeRuleFromStorage.getTemplateKey())
                                .orElse(null);
                            if (ruleOrTemplateDefinition == null) {
                                LOG.debug(
                                    "Rule {} is enabled on the server, but its template {} " +
                                    "is not available in SonarLint",
                            ...
                            ruleOrTemplateDefinition = analysisContext.get()
                                .findRule(activeRuleFromStorage.getRuleKey())
                                .orElse(null);
                            if (ruleOrTemplateDefinition == null) {
                                LOG.debug(
                                    "Rule {} is enabled on the server, but not available in SonarLint",
                                    activeRuleFromStorage.getRuleKey()
                                );
                                continue;
                            }
    ...
    private boolean shouldIncludeRuleForAnalysis(SonarLintRuleDefinition ruleDefinition) {
        return !ruleDefinition.getType().equals(RuleType.SECURITY_HOTSPOT)
            || serverConnection.permitsHotspotTracking();
    }
    ...
    private ServerActiveRule tryConvertDeprecatedKeys(ServerActiveRule possiblyDeprecatedActiveRuleFromStorage) {
        ...
        if (StringUtils.isNotBlank(possiblyDeprecatedActiveRuleFromStorage.getTemplateKey())) {
            ruleOrTemplateDefinition = analysisContext.get()
                .findRule(possiblyDeprecatedActiveRuleFromStorage.getTemplateKey())
                .orElse(null);
            if (ruleOrTemplateDefinition == null) {
                // The rule template is not known among our loaded analyzers,
                // so return it untouched, to let calling code take appropriate decision
                return possiblyDeprecatedActiveRuleFromStorage;
            }
            var ruleKeyPossiblyWithDeprecatedRepo = RuleKey.parse(
                possiblyDeprecatedActiveRuleFromStorage.getRuleKey()
            );
            var templateRuleKeyWithCorrectRepo = RuleKey.parse(
                ruleOrTemplateDefinition.getKey()
            );
            var ruleKey = new RuleKey(
                templateRuleKeyWithCorrectRepo.repository(),
                ruleKeyPossiblyWithDeprecatedRepo.rule()
            ).toString();
            return new ServerActiveRule(
                ruleKey,
                possiblyDeprecatedActiveRuleFromStorage.getSeverity(),
                possiblyDeprecatedActiveRuleFromStorage.getParams(),
                ruleOrTemplateDefinition.getKey()
            );
        } else {
            ruleOrTemplateDefinition = analysisContext.get()
                .findRule(possiblyDeprecatedActiveRuleFromStorage.getRuleKey())
                .orElse(null);
            if (ruleOrTemplateDefinition == null) {
                // The rule is not known among our loaded analyzers,
                // so return it untouched, to let calling code take appropriate decision
                return possiblyDeprecatedActiveRuleFromStorage;
            }
            return new ServerActiveRule(
    ...
    private static boolean isRuleFromExtraPlugin(Language ruleLanguage, ConnectedGlobalConfiguration config) {
        return config.getExtraPluginsPathsByKey().keySet().stream()
            .anyMatch(extraPluginKey -> ruleLanguage.getLanguageKey().equals(extraPluginKey));
    }
    ...
    @Override
    public CompletableFuture<ConnectedRuleDetails>
    getActiveRuleDetails(
        ...
        if (ruleDefFromPluginOpt.isPresent()) {
            ...
            if (globalConfig
                    .getExtraPluginsPathsByKey()
                    .containsKey(ruleDefFromPlugin.getLanguage().getPluginKey()) ||
                projectKey == null) {
                // if no project key, or for rules from extra plugins there will be no rules metadata in the storage
                return CompletableFuture.completedFuture(
                    new ConnectedRuleDetails(
                        ruleKey,
                        ruleDefFromPlugin.getName(),
                        ruleDefFromPlugin.getHtmlDescription(),
                        ruleDefFromPlugin.getDefaultSeverity(),
        ...
        if (projectKey != null) {
            ...
            if (storageActiveRule.isPresent()) {
                ...
                if (StringUtils.isNotBlank(activeRuleFromStorage.getTemplateKey())) {
                    var templateRuleDefFromPlugin = analysisContext.get()
                        .findRule(activeRuleFromStorage.getTemplateKey())
                        .orElseThrow(
                            () -> new IllegalStateException(
                                "Unable to find rule definition for rule template " +
                                activeRuleFromStorage.getTemplateKey()
                            )
                        );
                    return new ServerApi(new ServerApiHelper(endpoint, client))
                        .rules()
                        .getRule(activeRuleFromStorage.getRuleKey())
                        .thenApply(
                    ...
                    return new ServerApi(new ServerApiHelper(endpoint, client))
                        ...
                        .getRule(activeRuleFromStorage.getRuleKey())
                        .thenApply(
                            serverRule -> ruleDefFromPluginOpt
                            .map(
                                ruleDefFromPlugin -> new ConnectedRuleDetails(
                                    ruleKey,
                                    ruleDefFromPlugin.getName(),
                                    ruleDefFromPlugin.getHtmlDescription(),
                                    Optional.ofNullable(serverSeverity)
    ...
    @Override
    public Map<String, ServerProject> downloadAllProjects(
        EndpointParams endpoint,
        HttpClient client,
        @Nullable ClientProgressMonitor monitor
    ) {
        return wrapErrors(
            () -> serverConnection.downloadAllProjects(
                endpoint, client, new ProgressMonitor(monitor)
            )
        );
    }
    ...
    private static ProjectBranches toApi(
        org.sonarsource.sonarlint.core.serverconnection.ProjectBranches projectBranchesFromStorage
    ) {
        return new ProjectBranches(
            projectBranchesFromStorage.getBranchNames(),
            projectBranchesFromStorage.getMainBranchName()
        );
    }
    ...
    public List<ServerIssue> getServerIssues(
        ...
        return serverConnection.getServerIssues(
            projectBinding,
            branchName,
            ideFilePath
        );
    ...
    @Override
    public List<ServerTaintIssue> getServerTaintIssues(
        ProjectBinding projectBinding,
        String branchName,
        String ideFilePath
    ) {
        return serverConnection.getServerTaintIssues(
            projectBinding,
            branchName,
            ideFilePath
        );
    ...
    @Override
    public <G> List<G> getExcludedFiles(
        ProjectBinding projectBinding,
        Collection<G> files,
        Function<G, String> fileIdePathExtractor,
    ...
    public void subscribeForEvents(
        ...
        serverConnection.subscribeForEvents(
            endpoint,
            client,
            projectKeys,
            eventConsumer,
    ...
    @Override
    public void downloadAllServerIssuesForFile(
        EndpointParams endpoint,
        HttpClient client,
        ProjectBinding projectBinding,
        String ideFilePath,
        ...
        serverConnection.downloadServerIssuesForFile(
            endpoint,
            client,
            projectBinding,
            ideFilePath,
    ...
    @Override
    public void downloadAllServerTaintIssuesForFile(
        EndpointParams endpoint,
        HttpClient client,
        ProjectBinding projectBinding,
        String ideFilePath,
        ...
        serverConnection.downloadServerTaintIssuesForFile(
            endpoint,
            client,
            projectBinding,
            ideFilePath,
    ...
    public void downloadAllServerIssues(
        ...
        serverConnection.downloadServerIssuesForProject(
            endpoint,
            client,
            projectKey,
            branchName
    ...
    public void syncServerIssues(
        ...
        serverConnection.syncServerIssuesForProject(
            endpoint,
            client,
            projectKey,
            branchName
    ...
    @Override
    public void syncServerTaintIssues(
        EndpointParams endpoint,
        HttpClient client,
        String projectKey,
        String branchName,
        ...
        serverConnection.syncServerTaintIssuesForProject(
            endpoint,
            client,
            projectKey,
            branchName
    ...
    @Override
    public void downloadAllServerHotspots(
        EndpointParams endpoint,
        HttpClient client,
        String projectKey,
        String branchName,
        ...
        serverConnection.downloadAllServerHotspots(
            endpoint,
            client,
            projectKey,
            branchName,
    ...
    @Override
    public void downloadAllServerHotspotsForFile(
        EndpointParams endpoint,
        HttpClient client,
        ProjectBinding projectBinding,
        String ideFilePath,
        ...
        serverConnection.downloadAllServerHotspotsForFile(
            endpoint,
            client,
            projectBinding,
            ideFilePath,
    ...
    @Override
    public Collection<ServerHotspot> getServerHotspots(
        ProjectBinding projectBinding,
        String branchName,
        String ideFilePath
    ) {
        return serverConnection.getServerHotspots(
            projectBinding, branchName, ideFilePath
        );
    }
    ...
    public void updateProject(
        ...
        setLogging(null);
        serverConnection.updateProject(
            endpoint, client, projectKey, new ProgressMonitor(monitor)
        );
    }
    ...
    private static class AnalysisContext {
        ...
        public AnalysisContext(
            List<PluginDetails> pluginDetails,
            Map<String, SonarLintRuleDefinition> allRulesDefinitionsByKey,
            AnalysisEngine analysisEngine
        ) {
            this.pluginDetails = pluginDetails;
            this.allRulesDefinitionsByKey = allRulesDefinitionsByKey;
            this.analysisEngine = analysisEngine;
            this.deprecatedRuleKeysMapping = allRulesDefinitionsByKey
                .values()
                .stream()
