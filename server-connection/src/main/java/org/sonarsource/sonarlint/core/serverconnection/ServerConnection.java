...
public class ServerConnection {
    ...
    public ServerConnection(
        ...
        this.issuesUpdater = new ServerIssueUpdater(
            serverIssueStoresManager,
            new IssueDownloader(enabledLanguages),
            new TaintIssueDownloader(enabledLanguages)
        );
        ...
        this.storageSynchronizer = new LocalStorageSynchronizer(
            enabledLanguages,
            embeddedPluginKeys,
            serverInfoSynchronizer,
            pluginsStorage,
            projectStorage
        );
        ...
        coreEventRouter = new EventDispatcher()
            ...
            .dispatch(
                TaintVulnerabilityRaisedEvent.class,
                new UpdateStorageOnTaintVulnerabilityRaised(serverIssueStoresManager)
            )
            .dispatch(
                TaintVulnerabilityClosedEvent.class,
                new UpdateStorageOnTaintVulnerabilityClosed(serverIssueStoresManager)
            );
    ...
    public Map<String, ServerProject> downloadAllProjects(
        ...
        try {
            return new ServerApi(endpoint, client)
                .component()
                .getAllProjects(monitor)
                .stream()
                .collect(Collectors.toMap(ServerProject::getKey, p -> p));
        } catch (Exception e) {
    ...
    public List<ServerIssue> getServerIssues(
        ...
        return issueStoreReader.getServerIssues(
    ...
    public List<ServerTaintIssue> getServerTaintIssues(
        ...
        return issueStoreReader.getServerTaintIssues(
            projectBinding, branchName, ideFilePath
        );
    }
    ...
    public void subscribeForEvents(
        ...
        HttpClient client,
        Set<String> projectKeys,
        Consumer<ServerEvent> clientEventConsumer,
        ClientLogOutput clientLogOutput
    ) {
        serverEventsAutoSubscriber.subscribePermanently(
            new ServerApi(new ServerApiHelper(endpoint, client)),
            projectKeys,
            enabledLanguages,
    ...
    public ProjectBinding calculatePathPrefixes(String projectKey, Collection<String> ideFilePaths) {
        ...
        List<Path> sqPathList = storageReader
            .readProjectComponents(projectKey)
        ...
        return new ProjectBinding(
            projectKey,
            FilenameUtils.separatorsToUnix(match.sqPrefix().toString()),
            FilenameUtils.separatorsToUnix(match.idePrefix().toString())
        );
    }

    public void downloadServerIssuesForFile(
        EndpointParams endpoint,
        HttpClient client,
        ProjectBinding projectBinding,
        String ideFilePath,
        ...
        issuesUpdater.updateFileIssues(
            serverApi,
            projectBinding,
            ideFilePath,
            branchName,
    ...
    public void downloadServerTaintIssuesForFile(
        EndpointParams endpoint,
        HttpClient client,
        ProjectBinding projectBinding,
        String ideFilePath,
        ...
        var serverApi = new ServerApi(new ServerApiHelper(endpoint, client));
        var serverVersion = readOrSynchronizeServerVersion(serverApi);
        issuesUpdater.updateFileTaints(
            serverApi,
            projectBinding,
            ideFilePath,
            branchName,
    ...
    public void downloadServerIssuesForProject(
        EndpointParams endpoint,
        HttpClient client,
        String projectKey,
        String branchName
    ...
    public void downloadAllServerHotspots(
        ...
        var serverApi = new ServerApi(new ServerApiHelper(endpoint, client));
        hotspotsUpdater.updateAll(
            serverApi.hotspot(),
            projectKey,
            branchName,
            () -> readOrSynchronizeServerVersion(serverApi),
            progress
        );
    }
    ...
    public void syncServerIssuesForProject(
        EndpointParams endpoint,
        HttpClient client,
        String projectKey,
        String branchName
        ...
        if (IssueApi.supportIssuePull(isSonarCloud, serverVersion)) {
            LOG.info(
                "[SYNC] Synchronizing issues for project '{}' on branch '{}'",
                projectKey,
                branchName
            );
    ...
    public void syncServerTaintIssuesForProject(
        EndpointParams endpoint,
        HttpClient client,
        String projectKey,
        String branchName
        ...
        if (IssueApi.supportIssuePull(isSonarCloud, serverVersion)) {
            LOG.info(
                "[SYNC] Synchronizing taint issues for project '{}' on branch '{}'",
                projectKey,
                branchName
            );
    ...
    public void updateProject(
        ...
        projectStorageUpdateExecutor.update(
            new ServerApi(new ServerApiHelper(endpoint, client)),
            projectKey,
            monitor
        );
