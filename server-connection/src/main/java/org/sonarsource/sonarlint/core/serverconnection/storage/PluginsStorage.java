...
public class PluginsStorage {
        ...
        try {
            ...
            rwLock.write(
                () -> {
                    ...
                    var references = Files.exists(pluginsFilePath)
                        ? ProtobufUtil.readFile(pluginsFilePath, Sonarlint.PluginReferences.parser())
    ...
    public List<StoredPlugin> getStoredPlugins() {
        ...
        return rwLock.read(
            ...
            ? ProtobufUtil.readFile(filePath, Sonarlint.PluginReferences.parser())
            : Sonarlint.PluginReferences.newBuilder().build()
        ).getPluginsByKeyMap()
            .values().stream()
    ...
    public Map<String, Path> getStoredPluginPathsByKey() {
        return byKey(getStoredPlugins())
            .entrySet()
            .stream()
            .collect(Collectors.toMap(Map.Entry::getKey, v -> v.getValue().getJarPath()));
    }
    ...
    private List<File> getUnknownFiles() {
        ...
        var knownPluginsPaths = getStoredPlugins()
            .stream()
            .map(StoredPlugin::getJarPath).collect(Collectors.toSet());
        try (Stream<Path> pathsInDir = Files.list(rootPath)) {
            return pathsInDir
                .filter(p -> !p.equals(getPluginsFilePath()))
