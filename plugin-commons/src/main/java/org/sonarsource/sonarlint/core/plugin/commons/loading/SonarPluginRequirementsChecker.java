...
public class SonarPluginRequirementsChecker {
    ...
    SonarPluginRequirementsChecker(
        PluginsMinVersions pluginMinVersions,
        org.sonar.api.utils.Version pluginApiVersion
    ) {
    ...
    /**
     * Attempt to read JAR manifests, load metadata,
     * and check all requirements to ensure the plugin can be instantiated.
     */
    public Map<String, PluginRequirementsCheckResult> checkRequirements(
        ...
        Version jreCurrentVersion,
        boolean shouldCheckNodeVersion,
        ...
        for (Path jarLocation : pluginJarLocations) {
            ...
            if (resultsByKey.containsKey(plugin.getKey())) {
                throw new IllegalStateException(
                    ...
                    plugin.getJarFile() +
                    "' and '" +
                    resultsByKey.get(plugin.getKey()).getPlugin().getJarFile() +
                    "'"
                );
            }
            resultsByKey.put(
                plugin.getKey(),
                checkIfSkippedAndPopulateReason(
                    plugin,
                    enabledLanguages,
                    jreCurrentVersion,
                    shouldCheckNodeVersion,
                    nodeCurrentVersion
                )
            );
        }
        for (var e : resultsByKey.entrySet()) { // ???
            out.printf(
                "--- after first filter: %s, skip: %b\n",
                e.getKey(), e.getValue().isSkipped()
            );
        }
        ...
        for (PluginRequirementsCheckResult result : resultsByKey.values()) {
            if (!result.isSkipped()) {
                resultsByKey.put(
                    result.getPlugin().getKey(),
                    checkUnsatisfiedPluginDependency(result, resultsByKey)
                );
            }
        }
        for (var e : resultsByKey.entrySet()) { // ???
            out.printf(
                "--- second first filter: %s, skip: %b\n",
                e.getKey(), e.getValue().isSkipped()
            );
        }
        return resultsByKey;
    ...
    private PluginRequirementsCheckResult checkIfSkippedAndPopulateReason(
        PluginInfo plugin,
        Set<Language> enabledLanguages,
        Version jreCurrentVersion,
        boolean shouldCheckNodeVersion,
        ...
        // for (var e : enabledLanguages) { // ???
        //     out.printf(
        //         "--- enabled --- languageKey: %s, pluginKey: %s, label: %s\n",
        //         e.getLanguageKey(),
        //         e.getPluginKey(),
        //         e.getLabel()
        //     );
        ...
        for (var e : languages) { // ???
            out.printf(
                "--- target --- languageKey: %s, pluginKey: %s, label: %s\n",
                e.getLanguageKey(),
                e.getPluginKey(),
                e.getLabel()
            );
        ...
        if (!languages.isEmpty() && enabledLanguages.stream().noneMatch(languages::contains)) {
            if (languages.size() > 1) {
                out.printf(
                    "--- Plugin '%s' is excluded because none of languages '%s' are enabled. Skip loading it.\n",
                    plugin.getName(),
                    languages.stream().map(Language::toString).collect(Collectors.joining(","))
                );
                LOG.debug(
                    "Plugin '{}' is excluded because none of languages '{}' are enabled. Skip loading it.",
                    plugin.getName(),
                    languages.stream().map(Language::toString).collect(Collectors.joining(","))
                );
            } else {
                out.printf(
                    "--- Plugin '%s' is excluded because language '%s' is not enabled. Skip loading it.\n",
                    plugin.getName(),
                    languages.iterator().next().toString()
                );
                LOG.debug(
                    "Plugin '{}' is excluded because language '{}' is not enabled. Skip loading it.",
                    plugin.getName(),
                    languages.iterator().next()
                );
            }
            return new PluginRequirementsCheckResult(
                plugin, new SkipReason.LanguagesNotEnabled(languages)
            );
        }
        if (!isCompatibleWith(plugin, implementedPluginApiVersion)) {
            LOG.debug(
                "Plugin '{}' requires plugin API {} while SonarLint supports only up to {}. Skip loading it.",
                plugin.getName(),
                plugin.getMinimalSqVersion(),
                implementedPluginApiVersion.removeQualifier().toString()
            );
        ...
        if (pluginMinVersion != null && !pluginMinVersions.isVersionSupported(pluginKey, plugin.getVersion())) {
            LOG.debug(
                "Plugin '{}' version '{}' is not supported (minimal version is '{}'). Skip loading it.",
                plugin.getName(), plugin.getVersion(),
                pluginMinVersion
            );
            return new PluginRequirementsCheckResult(
                plugin,
                new SkipReason.IncompatiblePluginVersion(pluginMinVersion)
            );
        }
        ...
        if (jreMinVersion != null) {
            if (!jreCurrentVersion.satisfiesMinRequirement(jreMinVersion)) {
                LOG.debug(
                    "Plugin '{}' requires JRE {} while current is {}. Skip loading it.",
                    plugin.getName(), jreMinVersion, jreCurrentVersion
                );
                return new PluginRequirementsCheckResult(
                    plugin,
                    new SkipReason.UnsatisfiedRuntimeRequirement(
                        RuntimeRequirement.JRE,
                        jreCurrentVersion.toString(),
                        jreMinVersion.toString()
                    )
                );
            }
        }
        if (shouldCheckNodeVersion) {
            ...
            if (nodeMinVersion != null) {
                if (nodeCurrentVersion.isEmpty()) {
                    LOG.debug(
                        "Plugin '{}' requires Node.js {}. Skip loading it.",
                        plugin.getName(),
                        nodeMinVersion
                    );
                    return new PluginRequirementsCheckResult(
                        ...
                        new SkipReason.UnsatisfiedRuntimeRequirement(
                            ...
                            null,
                            nodeMinVersion.toString()
                        )
                    );
                } else if (!nodeCurrentVersion.get().satisfiesMinRequirement(nodeMinVersion)) {
                    LOG.debug(
                        "Plugin '{}' requires Node.js {} while current is {}. Skip loading it.",
                        plugin.getName(),
                        nodeMinVersion,
                        nodeCurrentVersion.get()
                    );
                    return new PluginRequirementsCheckResult(
                        plugin,
                        new SkipReason.UnsatisfiedRuntimeRequirement(
                            RuntimeRequirement.NODEJS,
                            nodeCurrentVersion.get().toString(),
                            nodeMinVersion.toString()
                        )
                    );
                }
    ...
    private static PluginRequirementsCheckResult checkUnsatisfiedPluginDependency(
        PluginRequirementsCheckResult currentResult,
        ...
        for (RequiredPlugin required : plugin.getRequiredPlugins()) {
            if ("license".equals(required.getKey()) ||
                (Language.JS.getPluginKey().equals(plugin.getKey()) &&
                 OLD_SONARTS_PLUGIN_KEY.equals(required.getKey()))) {
                // Workaround for SLCORE-259
                // This dependency was added to ease migration on SonarQube, but can be ignored on SonarLint
                // Note: The dependency was removed in SonarJS 6.3 but we should still keep the workaround
                //       as long as we want to support older versions
                continue;
            }
            var depInfo = currentResultsByKey.get(required.getKey());
            // We could possibly have a problem with transitive dependencies,
            // since we evaluate in no specific order.
            // A -> B -> C
            ...
            if (depInfo == null || depInfo.isSkipped()) {
                LOG.debug(
                    "Plugin '{}' dependency on '{}' is unsatisfied. Skip loading it.",
                    currentResult.getPlugin().getName(),
                    required.getKey()
                );
                return new PluginRequirementsCheckResult(
                    currentResult.getPlugin(),
                    new SkipReason.UnsatisfiedDependency(required.getKey())
                );
            }
        }
        ...
        if (basePluginKey != null) {
            ...
            if (baseInfo == null || baseInfo.isSkipped()) {
                ...
                return new PluginRequirementsCheckResult(
                    currentResult.getPlugin(),
                    new SkipReason.UnsatisfiedDependency(basePluginKey)
                );
            }
        }
